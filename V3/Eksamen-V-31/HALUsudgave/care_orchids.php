<?php
    ...
    $result = $db->query("SELECT * FROM flowers WHERE FClass='Orchid' ORDER BY RAND() LIMIT 5");
    if ($db->error) 
    {
        echo $db->error;
    }
    ...
    <main class="careorchids">
        ...
        <section class="careorchidsright">
            <article>
                ...
                <ul>
                <?php 
                    for ($i=0; $i < count($orchids) ; $i++) { 
                        if (isset($orchids[$i]["FImg"])) {
                            $orchidsImg = $orchids[$i]["FImg"];
                        }
                        else
                        {
                            $orchidsImg = "imagecomingsoon.png";
                        }
                        ?>
                        <li>
                            <a href="#">
                                <img src="img/<?php echo $orchidsImg; ?>" alt="<?php echo $orchids[$i]["FAlttxt"]?>">
                                <h3><?php echo $orchids[$i]["FName"]?></h3>
                                <p>From $<?php echo $orchids[$i]["FPrice"]?></p>
                            </a>
                        </li>
                        <?php
                    }
